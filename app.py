...
setup_disc_input = sc2.number_input(" ", value=setup_disc_slider, step=0.5, format="%.2f")
...
ship_disc_input = sh2.number_input(" ", value=ship_disc_slider, step=0.5, format="%.2f")
...
hand_disc_input = hd2.number_input(" ", value=hand_disc_slider, step=0.5, format="%.2f")
...
growth_input = g2.number_input(" ", value=growth_slider, step=1.0)
...
with col2:
    ...
    st.metric("GM $", f"${n_gm:,.2f}", delta=f"{'🔼' if gm_delta >= 0 else '🔻'} ${abs(gm_delta):,.2f}")
    st.metric("GM %", f"{n_gmp*100:.1f}%", delta=f"{'🔼' if gm_pct_delta >= 0 else '🔻'} {abs(gm_pct_delta*100):.1f}%")
