...
with col2:
    ...
    st.metric("GM $", f"${n_gm:,.2f}", delta=f"${abs(gm_delta):,.2f}", delta_color="normal" if gm_delta == 0 else ("inverse" if gm_delta < 0 else "off"))
    st.metric("GM %", f"{n_gmp*100:.1f}%", delta=f"{abs(gm_pct_delta*100):.1f}%", delta_color="normal" if gm_pct_delta == 0 else ("inverse" if gm_pct_delta < 0 else "off"))
