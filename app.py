...
# Safely check and parse product ID column
if "Five_Digit" not in data.columns:
    st.error("The 'Five_Digit' column was not found in your Excel file.")
    st.stop()

product_options = pd.to_numeric(data["Five_Digit"], errors="coerce")
product_options = product_options.dropna().astype("int", errors="ignore").unique()
product_options = sorted(product_options)

if not product_options:
    st.error("No valid product IDs found. Please check your Excel file.")
    st.stop()

product_id = st.sidebar.selectbox("Select Product (Five_Digit ID)", product_options)
