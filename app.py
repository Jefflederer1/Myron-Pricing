...
st.title("Myron Pricing Tool – v4.4")
...
item_description = item_row.get("Item_Description", "No Description")
...
discount_col1, discount_col2 = st.sidebar.columns([2, 1])
discount_slider = discount_col1.slider("Discount % (applies to all qty)", 0.0, 100.0, 20.0, step=0.5)
discount_input = discount_col2.number_input(" ", value=discount_slider, step=0.5, format="%.2f")
...
# Shipping & Handling
st.sidebar.header("Shipping / Handling")
...
# --- SCENARIO CALCULATION BASED ON FIRST QTY BREAK ---
...
with col2:
    ...
    st.metric("GM $", f"${new_gm:,.2f}", delta=f"{gm_delta(new_gm - orig_gm)} ${abs(new_gm - orig_gm):,.2f}")
    st.metric("GM %", f"{new_gmp*100:.1f}%", delta=f"{gm_delta(new_gmp - orig_gmp)} {(new_gmp - orig_gmp)*100:.1f}%")
